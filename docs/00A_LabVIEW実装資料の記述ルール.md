...
単純なSubVIでも、少なくとも`1～3`を記載する。単体テスト値を定義できる場合は`4`も記載する。

### 2.1 目的と処理概要

- このVIが必要な理由を書く。
- 入力を何へ変換し、何を出力するかを書く。
- Forループを使う場合は「何を1反復で処理するか」を書く。
- シフトレジスタを使う場合は「何を反復間で保持するか」を書く。
...
関数名は必ず**日本語名（英語名）**の順で記載する。パレット位置は日本語版LabVIEWを基準とし、版により異なる場合は次を併記する。
...
LabVIEW画面上の`x78`は、資料中の`0x78`と同じ値である。
...
- 配列表示器に10セルしか見えなくても、配列自体が24要素なら正常である。
...
入力値検証を既存エラー確認より外側へ置き、元エラーを上書きしない。
...
`N`端子を未配線にする場合は、自動指標付け配列の要素数だけ反復することを書く。

### 5.3 配列全体をループ内で使う場合

`SYSINFO Raw U8[960]`のように、各反復で配列全体から任意位置を切り出す場合は、自動指標付けを無効にする。
...
### 6.4 Falseケースで初期定数へ戻さない

初期値`-1`は左外側端子へ一度だけ接続する。判定Falseケースでは、左内側端子の現在値を右内側端子へそのまま渡す。
...
### 6.5 ループ終了後は右外側端子を使用する

単一の最終値を返す場合は、通常の自動指標付け出力トンネルではなくシフトレジスタ右外側端子を表示器へ接続する。太い青線のI32配列をI32単体表示器へ接続しない。
...
比較に使用する値が、部分配列のU8[4]なのか、変換SubVI後のI32なのかを必ず明記する。
...
- `Value`だけでなく、`I32_To_LE_U8x4.vi`の`Value`入力端子と書く。
- 出力へ用途名を付ける。
...
構造体のダミーバイト列は、手で大量入力せず、テスト用VIまたは一時生成回路で作る。
...
`Replace Array Subset`へ接続するものを明記する。

# 10. TestStand シーケンス構築手順

LabVIEW VI を呼び出し、試験全体のフロー・条件・タイミングを管理する。

## 10.1 TestStand の基本構造
...
- **Main**：試験本体（手順）。
- **Cleanup**：**異常時も含め必ず実行**される。→ 安全シャットダウンをここに置く（[11](./11_異常系処理とシャットダウン設計.md)）。

## 10.2 LabVIEW Adapter の設定
...
（[07_負荷電流VIと並列処理.md](./07_負荷電流VIと並列処理.md) と対応）
...
- Cleanup で安全シャットダウン（[11](./11_異常系処理とシャットダウン設計.md)）を必ず実行する。

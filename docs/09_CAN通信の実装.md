...
チェックサムのアルゴリズム・バイト位置・カウンタ幅は ECU 側の通信仕様書確認が必要だが、
**その部分を独立したサブVIに閉じ込めれば、他の配線（CLFN・バッファ構築・ループ構造）は
仕様確定前に着手できる**。

| 分類 | 内容 | 着手可否 |
|------|------|:---:|
| 分かっている部分 | RAMScope CLFN 配線、`SEND_SCENARIO`/`CANSEND_170_DATA` バッファ組み立て、ループ構造、TestStand I/O、共通仕様（[05](./05_VI設計方針と共通仕様.md)）| ✅ 先行着手可 |
| 未確定な部分 | チェックサム計算式、カウンタ／チェックサムのバイト・ビット位置、カウンタのビット幅 | ❌ 仕様確定後に差し替え |

**推奨構成：**

- **`CAN_Alive_Checksum_Calc.vi`（プレースホルダとして先行作成）**：
  入力＝カウンタ値・固定ペイロード、出力＝カウンタバイト・チェックサムバイト。
  中身は仮実装（例：単純 XOR）にしておき、仕様確定後にロジックのみ差し替える。
  呼び出し側の配線には影響しない。
- **`CAN_Send_Alive_Frame.vi` の骨格**：`For i = 0 to N-1` でこのサブVIを呼び、
  `SEND_SCENARIO_STEP` 配列に格納 → `ScenarioSendSet` へ CLFN 配線。
  ペイロードのバイト位置・カウンタ幅 `N` は**入力端子でパラメータ化**し、ハードコードしない。
- ダミー値（仮カウンタ・仮チェックサム）で先に `ScenarioSendSet`→`ScenarioSendStart` を
  発行し、CLFN 配線自体の動作を **CAN バスモニタ（CANalyzer 等）で意図通りのフレームが
  出るか**先に検証しておける（チェックサム内容の正しさとは切り離して検証できる）。
- 送信方式（方式A/B）を差し替え可能な設計にしておけば、カウンタ幅判明後の方式選択の
  変更も吸収しやすい（`CAN_Alive_Checksum_Calc.vi` は共通、送信ループ側だけ差し替え）。

### 9.9.1 ECU 仕様の確認チェックリスト（実装前に確認する項目）

| # | 確認項目 | 内容 |
|---|---------|------|
| ① | CAN ID・DLC・送信周期 | 標準/拡張ID・ペイロード長・周期(ms) |
| ① | バイトレイアウト | ペイロード内のカウンタ・チェックサム・固定値のバイト/ビット位置マップ |
| ② | カウンタのビット幅 | 4bit(16通り)か8bit(256通り)か等。**方式A/Bの選択に直結**（`Step[64]`上限）|
| ② | カウンタの初期値・ラップ挙動 | 0開始か、最大値からのロールオーバーか |
| ③ | チェックサムのアルゴリズム | 単純XOR/加算か、CRC(多項式指定)か、AUTOSAR E2E(Profile 1/2/5/6/11等)か |
| ③ | CRC使用時のパラメータ | 多項式・初期値・最終XOR値・ビット順（MSB/LSB first）|
| ③ | チェックサムの保護範囲 | 計算対象バイト。CAN ID自体を含むか（AUTOSAR E2Eでは含むケースが多い）|
| ④ | 固定値の前提確認 | カウンタ・チェックサム以外の全バイトが試験を通じて固定か（方式Aの成立条件）|
| ⑤ | RAMScope側の設定 | 送信対象 `MdlNo`・物理チャンネル（Ch1/Ch2）、CAN 2.0B/FD（`isUseFDFormat`）|
